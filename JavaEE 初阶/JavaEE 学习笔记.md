...
1. 接收端会将自己可以接受的滑动窗口大小放入 TCP 协议端头的“窗口”字段中，并以 ACK 应答报文的方式通知发送端，告知发送端滑动窗口大小不得超过我所规定的值，发送端就会调整自己的滑动窗口大小，因此滑动窗口大小由接受端控制。接收段同时也会整理好缓冲区的数据（按照数据报序号进行组合，等待应用层的读取）
2. 接收端一旦发现自己的接收缓冲区快满时，就会把窗口大小设置成一个更小的值并以 ACK 应答报文的方式发送给发送端。
3. 发送端接收到了 ACK 报文后，就会调整自己的滑动窗口大小，降低自己的发送速度。
4. 如果接收端的接收缓冲区满了的话，就会直接把窗口大小设置成 0，暂停发送端的数据发送，但是接收段会发送一个窗口探测报文（窗口探测报文不携带任何载荷，只单纯地做询问）询问接收端是否已经处理好了数据，是否可以重新开启发送。
...
2. **滑动窗口（发送端没有接收到 ACK？接收端没有接收到数据包？ ）**

3. **流量控制（根据自身缓冲区限制滑动窗口大小）**

4. **拥塞控制（轮回控制流量）**

5. **延迟应答（捎带应答）**

6. **粘包问题（面向字节流的必然结果，在报文首部添加报文的字节长度）**
