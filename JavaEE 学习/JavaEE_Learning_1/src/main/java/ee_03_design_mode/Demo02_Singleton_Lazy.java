...
public class Demo02_Singleton_Lazy {
    // 懒汉
    // 延时加载
    ...
    private static volatile Demo02_Singleton_Lazy singletonLazy = null;
    public String info;

    private Demo02_Singleton_Lazy(){
        info = "Hello, Singleton_Lazy!";
    }

    public static Demo02_Singleton_Lazy getInstance(){
        ...
        if (singletonLazy == null) {
            synchronized (Demo02_Singleton_Lazy.class) {
                if (singletonLazy == null) {
                    // 达到了延时加载
                    System.out.println("第一次访问单例，懒汉单例即将创建...");
                    singletonLazy = new Demo02_Singleton_Lazy();
                }
            }
        }
